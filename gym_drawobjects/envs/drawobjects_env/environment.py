...
class DrawObjectsEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.current_canvas = self.create_new_canvas()
    ...
    def redraw(self):
        if not self._is_rendering:
            return
        self.gui.update(self.current_canvas, self.last_reward,
                        truncate(LABELS[self.label_idx-1]))
    ...
    def apply_action(self, action):
        if action == UP:
            self.current_pos = (
                self.current_pos[0], max(0, self.current_pos[1]-1))

        elif action == DOWN:
            self.current_pos = (self.current_pos[0], min(
                self.height-1, self.current_pos[1]+1))

        elif action == RIGHT:
            self.current_pos = (
                min(self.width-1, self.current_pos[0]+1), self.current_pos[1])

        elif action == LEFT:
            self.current_pos = (
                max(0, self.current_pos[0]-1), self.current_pos[1])
    ...
    def reward(self, action):
        self.last_reward = inception.get_prediction(
            self.current_canvas, self.label_idx)
